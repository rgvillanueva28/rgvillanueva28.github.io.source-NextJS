...
export default function CardOne({ onTop }) {
  return (
    <div className="flex flex-col text-center container-sm p-10 sm:px-12 md:px-20 lg:px-32 rounded-md lg:flex lg:flex-row min-h-screen justify-center">
      ...
      <div className="text-left lg:pt-0 min-h-full lg:justify-center lg:flex lg:flex-col lg:mr-64 lg:pr-20">
        ...
        <p className="text-accent-light text-base mb-5">
          ...
          I am interested to learn the fields of Cybersecurity, Web Development, Mobile Development, Data Analysis and Visualization, and Artificial Intelligence.
          I have also created several projects.
          Check out the <Link href="/portfolio"><a className="text-foreground opacity-100 hover:opacity-75">portfolio</a></Link> tab for more information about these projects.
        </p>
        {/* <Link href="#contact">
        ...
        <ScrollLink
              ...
              className={"cursor-pointer mr-auto transition duration-200 ease-in-out py-2 px-4 bg-transparent rounded-md text-accent-light border-2 border-accent-light hover:bg-accent-light hover:text-foreground focus:outline-none " + (onTop ? "animate-bounce hover:animate-none" : "")}
            >Contact
        </ScrollLink>
      </div>
